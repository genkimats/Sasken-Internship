...
PITCH_THRESHOLD_POS = 5   # looking down
PITCH_THRESHOLD_NEG = -15  # looking up

ROLL_THRESHOLD_POS = 15    # tilt right
ROLL_THRESHOLD_NEG = -10   # tilt left


def compute_pitch_roll_from_5pts(kp):
    """
    kp: dict with keys 'left_eye', 'right_eye', 'nose', 'mouth_left', 'mouth_right'
        each value is [x, y]
    Returns (pitch_deg, roll_deg)
    """
    L = np.array(kp['left_eye'], dtype=np.float32)
    R = np.array(kp['right_eye'], dtype=np.float32)
    N = np.array(kp['nose'], dtype=np.float32)
    ML = np.array(kp['mouth_left'], dtype=np.float32)
    MR = np.array(kp['mouth_right'], dtype=np.float32)
    ...
    eye_vec = R - L  # x to the right, y down (OpenCV)
    roll_rad = math.atan2(eye_vec[1], eye_vec[0])
    roll_deg = math.degrees(roll_rad)

    # --- PITCH (heuristic): vertical displacement of nose from eye line,
    # normalized by inter-ocular distance. Positive when looking down
    # (because y increases downward on images).
    eye_dist = np.linalg.norm(eye_vec) + 1e-6
    pitch_rad = math.atan2((N[1] - E_mid[1]), eye_dist)
    pitch_deg = math.degrees(pitch_rad)
    pitch_deg -= 28.0   # <-- normalize so default ~0°


    # Optional: stabilize by blending with mouth line
    # Uncomment to slightly smooth pitch using mouth position
    # pitch2_rad = math.atan2((M_mid[1] - E_mid[1]), eye_dist)
    # pitch_deg = 0.7 * pitch_deg + 0.3 * math.degrees(pitch2_rad)

    return pitch_deg, roll_deg

def draw_pitch_roll(frame, pitch_deg, roll_deg, x0=20, y0=120):
    # Check pitch status
    pitch_ok = PITCH_THRESHOLD_NEG <= pitch_deg <= PITCH_THRESHOLD_POS
    roll_ok  = ROLL_THRESHOLD_NEG <= roll_deg <= ROLL_THRESHOLD_POS

    pitch_color = (0, 255, 0) if pitch_ok else (0, 0, 255)
    roll_color  = (0, 255, 0) if roll_ok  else (0, 0, 255)

    cv2.putText(frame, f"Pitch: {pitch_deg:+.1f} deg", (x0, y0),
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, pitch_color, 2)
    cv2.putText(frame, f"Roll : {roll_deg:+.1f} deg", (x0, y0+30),
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, roll_color, 2)

    # Alert messages when exceeding thresholds
    if pitch_deg < PITCH_THRESHOLD_NEG:
        cv2.putText(frame, "PITCH UP ALERT!", (x0, y0+65),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
    elif pitch_deg > PITCH_THRESHOLD_POS:
        cv2.putText(frame, "PITCH DOWN ALERT!", (x0, y0+65),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)

    if roll_deg < ROLL_THRESHOLD_NEG:
        cv2.putText(frame, "ROLL LEFT ALERT!", (x0, y0+95),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
    elif roll_deg > ROLL_THRESHOLD_POS:
        cv2.putText(frame, "ROLL RIGHT ALERT!", (x0, y0+95),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
...
CNN_CONSEC_FRAMES = 2
# --- Predict function ---
def predict_blink(left_eye, right_eye, frame):
    ...
    global blink_count, blink_detected, total_blinks
    if (left_prediction[0][0] > left_prediction[0][1] and right_prediction[0][0] > right_prediction[0][1]):
        if not blink_detected:
            blink_count += 1
            blink_detected = True
            cv2.putText(frame, "Blink Detected", (20, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
    else:
        blink_detected = False

    cv2.putText(frame, f"Total Blinks = {blink_count}", (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
...
while True:
    ...
    # --- Head pose from 5 landmarks (MTCNN) ---
    kps = result[0][0]['keypoints']  # has left_eye, right_eye, nose, mouth_left, mouth_right
    pitch_deg, roll_deg = compute_pitch_roll_from_5pts(kps)
    draw_pitch_roll(frame, pitch_deg, roll_deg, x0=20, y0=120)

    # (Optional) draw helper lines for visualization
    L = tuple(map(int, kps['left_eye']))
    R = tuple(map(int, kps['right_eye']))
    N = tuple(map(int, kps['nose']))
    ML = tuple(map(int, kps['mouth_left']))
    MR = tuple(map(int, kps['mouth_right']))
    cv2.line(frame, L, R, (255, 255, 255), 1)                 # eye line
    cv2.line(frame, ((L[0]+R[0])//2, (L[1]+R[1])//2), N, (255, 255, 255), 1)  # eye-mid to nose
    cv2.circle(frame, N, 2, (255, 255, 255), -1)
