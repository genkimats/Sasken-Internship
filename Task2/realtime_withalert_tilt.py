...
model = tf.keras.models.load_model("./ft_stage2_final_new.h5")

yawn_model = tf.keras.models.load_model("./yawn_new_model.h5")

blink_count = 0

blink_close_start_time = None  # Track when eyes first close
...
YAW_POS_THRESHOLD = 15    # Rightward yaw
YAW_NEG_THRESHOLD = -15   # Leftward yaw
PITCH_POS_THRESHOLD = 10   # Downward pitch
PITCH_NEG_THRESHOLD = -5  # Upward pitch
ROLL_POS_THRESHOLD  = 15   # Tilt right
ROLL_NEG_THRESHOLD  = -15  # Tilt left

# Thresholds in degrees
# YAW_THRESHOLD   = 15   # Left/right rotation
# PITCH_THRESHOLD = 15   # Up/down tilt
# ROLL_THRESHOLD  = 15   # Head tilt sideways

# ==== 3D canonical points for a generic face model ====
# Units: millimeters, from a standard 3DMM mean face model
MODEL_POINTS = np.array([
    [ -30.0,  -30.0,   0.0],  # Left eye
    [  30.0,  -30.0,   0.0],  # Right eye
    [   0.0,    0.0,   0.0],  # Nose tip
    [ -20.0,   30.0,   0.0],  # Left mouth corner
    [  20.0,   30.0,   0.0]   # Right mouth corner
], dtype=np.float32)


def compute_head_pose(kps):
    """
    ...
    pitch  = np.degrees(np.arctan2(rmat[1, 2], rmat[2, 2]))
...
def draw_angles_overlay(frame, yaw, pitch, roll):
    """
    Draw yaw, pitch, and roll angles on the right side of the frame.
    Highlights red when exceeding thresholds.
    Uses negative and positive thresholds for each angle.
    """
    h, w = frame.shape[:2]
    global tilt_start_time

    # Check thresholds and choose colors
    yaw_color = (0, 255, 0) if (YAW_NEG_THRESHOLD <= yaw <= YAW_POS_THRESHOLD) else (0, 0, 255)
    pitch_color = (0, 255, 0) if (PITCH_NEG_THRESHOLD <= pitch <= PITCH_POS_THRESHOLD) else (0, 0, 255)
    roll_color = (0, 255, 0) if (ROLL_NEG_THRESHOLD <= roll <= ROLL_POS_THRESHOLD) else (0, 0, 255)

    # Set starting coordinates on the right side
    x_offset = w - 220   # Distance from right edge
    y_start = 50         # Top margin
    line_spacing = 30    # Vertical spacing between lines

    # Draw angles on the right side
    cv2.putText(frame, f"Yaw:   {yaw:.1f} deg",   (x_offset, y_start), cv2.FONT_HERSHEY_SIMPLEX, 0.7, yaw_color, 2)
    cv2.putText(frame, f"Pitch: {pitch:.1f} deg", (x_offset, y_start + line_spacing), cv2.FONT_HERSHEY_SIMPLEX, 0.7, pitch_color, 2)
    cv2.putText(frame, f"Roll:  {roll:.1f} deg",  (x_offset, y_start + 2 * line_spacing), cv2.FONT_HERSHEY_SIMPLEX, 0.7, roll_color, 2)

    # If any angle exceeds threshold, show ALERT below the angles
    if not (YAW_NEG_THRESHOLD <= yaw <= YAW_POS_THRESHOLD) or \
       not (PITCH_NEG_THRESHOLD <= pitch <= PITCH_POS_THRESHOLD) or \
       not (ROLL_NEG_THRESHOLD <= roll <= ROLL_POS_THRESHOLD):
        if tilt_start_time is None:
            tilt_start_time = time.time()
        cv2.putText(frame, "ALERT: Head Pose Unsafe!", 
                    (x_offset - 100, y_start + 4 * line_spacing),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 3)
    else:
        tilt_start_time = None

    check_alert(frame, fps, tilt_start_time)
...
# --- Alert function ---
def check_alert(frame, fps, blink_close_start_time):
    # Use time instead of frame count for alert
    ALERT_DURATION = 2.0  # seconds
    if blink_close_start_time is not None:
        elapsed = time.time() - blink_close_start_time
        print(f"Eyes closed for {elapsed:.2f} seconds")
        if elapsed >= ALERT_DURATION:
            if not pygame.mixer.get_busy():  # Prevent repeated alerts
                alert_sound.play()
...
def predict_blink(left_eye, right_eye, frame, fps=30):
    global blink_count, blink_detected, blink_close_start_time, total_blinks

    left_tensor = preprocess(left_eye)
    right_tensor = preprocess(right_eye)
    
    if left_tensor is None or right_tensor is None:
        print("[WARN] Skipping blink prediction: invalid eye crop")
        return  # Don't call model.predict()
    ...
    if closed:
        if blink_close_start_time is None:
            blink_close_start_time = time.time()
        ...
        blink_close_start_time = None
        blink_detected = False

    # Use new alert logic
    check_alert(frame, fps, blink_close_start_time)
...
def predict_yawn(mouth, frame):
    ...
    if mouth_tensor is None:
        print("[WARN] Skipping yawn prediction: invalid mouth crop")
        return
...
while True:
    ...
    # --- Compute & draw pitch/roll from the 5-point landmarks we already have ---
    if len(result[0]) > 0:
        # Use ONNX MTCNN's 5-point landmarks directly
        kps = [
            tuple(result[0][0]['keypoints']['left_eye']),
            tuple(result[0][0]['keypoints']['right_eye']),
            tuple(result[0][0]['keypoints']['nose']),
            tuple(result[0][0]['keypoints']['mouth_left']),
            tuple(result[0][0]['keypoints']['mouth_right'])
        ]


        # Compute 3D head pose
        yaw, pitch, roll = compute_head_pose(kps)

        # Draw overlay
        draw_angles_overlay(frame, yaw, pitch, roll)
